    ...
    try:
        ...
        try:
            ...
            if not isinstance(criteria_list, list):
                ...
                raise ValueError("Criteria extraction returned invalid format (expected list)")
                
            # Filter out empty or non-string criteria
            criteria_list = [criterion for criterion in criteria_list if isinstance(criterion, str) and criterion.strip()]
            ...
            # Log the criteria
            logger.info(f"Extracted {len(criteria_list)} criteria in {time.time() - start_time:.2f} seconds")
