...
def generate_csv_from_scores(
    results: List[Dict[str, Any]], criteria_list: List[str]
) -> StreamingResponse:
    """
    ...
    if "error" not in df.columns:
        if len(criteria_list) > 0:
            ...
            score_columns = [
                ...
                if col in criteria_list
                or col.replace("[Required] ", "").replace("[Preferred] ", "")
                in [
                    c.replace("[Required] ", "").replace("[Preferred] ", "")
