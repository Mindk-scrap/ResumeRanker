...
async def score_resume_against_criteria(
    resume_content: str, criteria: List[str]
) -> Dict[str, int]:
    """
    ...
    try:
        ...
        result = resume_ranker_crew.kickoff(
            {"resume_content": resume_content, "criteria": criteria}
        )
        ...
        try:
            ...
            try:
                ...
                if matches:
                    ...
                    logger.info(
                        f"Reconstructed JSON with {len(scores_objects)} complete score objects"
                    )
                    ...
                    if criteria_matches or score_matches:
                        ...
                        result_dict = {"scores": scores_objects}
                        logger.info(
                            f"Recovered {len(scores_objects)} criteria-score pairs using pattern matching"
                        )
                    else:
                        ...
                        if number_matches and len(criteria) > 0:
                            ...
                            result_dict = {"scores": scores_objects}
                            logger.warning(
                                f"Last resort recovery: matched {min_length} numbers to criteria"
                            )
                        else:
                            logger.error(
                                "Could not recover any score data: no valid patterns found"
                            )
                            result_dict = {"scores": []}
            ...
            for score_obj in result_dict["scores"]:
                ...
                if not isinstance(score, (int, float)) or score < 0 or score > 5:
                    logger.warning(
                        f"Invalid score value for {criterion}: {score}, clamping to valid range"
                    )
                    score = max(
                        0, min(5, int(score) if isinstance(score, (int, float)) else 0)
                    )
            ...
            logger.warning(
                "Using default scores (0) for all criteria due to parsing error"
            )
