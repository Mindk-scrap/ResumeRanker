...
class DocumentProcessor:
    ...
    def extract_text(self, content: bytes, file_ext: str) -> str:
        ...
        try:
            ...
            if file_ext.lower() not in extension_map:
                raise ValueError(
                    f"Unsupported file extension: {file_ext}. Supported: {', '.join(extension_map.keys())}"
                )
    ...
    async def extract_text_from_upload(self, file: UploadFile) -> str:
        ...
        try:
            ...
            if mime_type not in self.SUPPORTED_MIME_TYPES:
                ...
                raise ValueError(
                    f"Unsupported file type. Only PDF and DOCX files are supported."
                )
    ...
    def _extract_from_pdf(self, content: bytes, filename: str) -> str:
        ...
        try:
            ...
            logger.info(
                f"Extracting text from PDF: {filename} with {len(reader.pages)} pages"
            )
    ...
    def _extract_from_docx(self, content: bytes, filename: str) -> str:
        ...
        try:
            ...
            logger.info(
                f"Extracting text from DOCX: {filename} with {len(doc.paragraphs)} paragraphs"
            )
