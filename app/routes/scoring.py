...
async def score_resumes(
    criteria: str = Form(
        ...,
        description="JSON array of ranking criteria strings or comma-separated list of criteria",
    ),
    ...
    try:
        ...
        try:
            ...
            if isinstance(criteria_parsed, list):
                ...
                raise ValueError(
                    "Invalid criteria format, expected array or object with 'criteria' key"
                )
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid criteria format: {str(e)}"
        )
    ...
    for file in resumes:
        try:
            ...
            candidate_name = (
                ...
                if name_result and name_result.get("confidence", 0) >= 70
                else None
            )
            ...
            for criterion, score in scores.items():
                ...
                column_name = criterion.replace("[Required] ", "").replace(
                    "[Preferred] ", ""
                )
